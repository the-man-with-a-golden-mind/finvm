  ...
  let
    ...
    pendingKind =
      case Map.lookup "key" fields, Map.lookup "payload" fields of
        Just (VString _), Just _ -> PendingAwaitReply
        _, _ -> PendingTransport
    pidVal =
      case pendingKind of
        PendingAwaitReply -> getStr "pid"
        PendingTransport -> case Map.lookup "pid" fields of
          Just (VString s) -> s
          _ -> ""
    keyVal =
      case pendingKind of
        PendingAwaitReply -> getStr "key"
        PendingTransport -> ""
    payloadVal =
      case pendingKind of
        PendingAwaitReply -> getVal "payload"
        PendingTransport -> e.payload
    kindVal =
      case pendingKind of
        PendingAwaitReply -> "await_reply"
        PendingTransport -> "transport"
  in objectJson
    [ Tuple "kind" (Json.fromString kindVal)
...
decodeDeliveries :: String -> Either String (Array ResumeDelivery)
decodeDeliveries s =
  if s == "" then Right []
  else do
    root <- jsonParser s
...
applyDelivery m d = case d of
  ...
  DeliveryMailboxMessage payload -> case Scheduler.findProcess m.scheduler payload.pid of
    ...
    Just p ->
      ...
      in m { scheduler = s2 }
  DeliveryEffectReply payload -> case Scheduler.findProcess m.scheduler payload.pid of
    Nothing -> m
    Just p ->
      let
